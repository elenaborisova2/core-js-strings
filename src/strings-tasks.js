 ...
 *   removeLeadingAndTrailingWhitespaces('cat ')z => 'cat'
...
function removeFirstOccurrences(str, value) {
  ...
  let s = str;
  if (index !== -1) {
    s = str.slice(0, index) + str.slice(index + value.length);
  }
  return s;
}
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((x) => {
    if (getStringLength(x) > getStringLength(word)) {
      word = x;
    }
  });
  return word;
...
function invertCase(str) {
  ...
  str.split('').forEach((x) => {
    if (x === x.toLowerCase()) {
      newLetters += x.toUpperCase();
    } else {
      newLetters += x.toLowerCase();
    }
  });
  return newLetters;
}
...
function getCardId(value) {
  ...
  const m = sym.indexOf(value.slice(-1));
  const n = card.indexOf(value.slice(0, -1));
  return m * 13 + n;
}
